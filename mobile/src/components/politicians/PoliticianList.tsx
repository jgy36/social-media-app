...
import { RefreshCw, AlertCircle, ChevronDown, Search } from 'react-native-vector-icons/Feather';
...
export const PoliticianList: React.FC<PoliticianListProps> = ({
  ...
  const renderCountySection = () => (
    <View className="mb-4">
      ...
      <FlatList
        ...
        renderItem={({ item }) => <PoliticianCard key={item.id} politician={item} />}
  ...
  if (error) {
    return (
      <View className="flex-1 items-center justify-center p-4">
        <AlertCircle name="alert-circle" size={48} color="#EF4444" className="mb-4" />
        <Text className="text-red-500 font-medium mb-2 text-center">Error loading politicians</Text>
        <Text className="text-gray-500 dark:text-gray-400 mb-4 text-center">{error}</Text>
        {onRetry && (
          <TouchableOpacity 
            onPress={onRetry} 
  ...
  if (isLoading) {
    return (
      <View className="flex-1 items-center justify-center">
        ...
        <Text className="mt-4 text-gray-500 dark:text-gray-400">Loading politicians...</Text>
      </View>
    );
  }
  ...
  if (!selectedCounty) {
    return (
      <View className="flex-1 items-center justify-center p-4">
        <ChevronDown name="chevron-down" size={48} color="gray" className="mb-4" />
  ...
  if (politicians.length === 0) {
    return (
      <View className="flex-1 items-center justify-center p-4">
        <Search name="search" size={48} color="gray" className="mb-4" />
        <Text className="text-gray-500 dark:text-gray-400 text-center">
    ...
    <View className="flex-1 bg-white dark:bg-gray-900">
      <View className="px-4 py-4 border-b border-gray-200 dark:border-gray-700">
        <Text className="text-xl font-bold text-gray-900 dark:text-white">
          Politicians for{' '}
          <Text className="text-blue-500">{selectedCounty}, {selectedState}</Text>
        </Text>
      </View>
      ...
      <FlatList
        data={[
          ...
          if (item.type === 'county-header' && countyPoliticians.length === 0) return false;
          if (item.type === 'state-header' && statePoliticians.length === 0) return false;
          return true;
        })}
        renderItem={({ item }: any) => {
          if (item.type === 'header') {
        ...
        keyExtractor={(item: any, index) => 
          item.type === 'politician' ? item.data.id.toString() : `${item.type}-${index}`
        }
        showsVerticalScrollIndicator={false}
