...
const PoliticianCard: React.FC<PoliticianCardProps> = ({ politician }) => {
  ...
  return (
    <View className={`rounded-lg shadow-md border m-2 p-4 ${getPartyBackground(politician.party)}`}>
      <View className="mb-2">
        <Text className="text-lg font-bold text-gray-900 dark:text-white">{politician.name}</Text>
        <Text className="text-sm text-gray-600 dark:text-gray-400">{politician.position}</Text>
      </View>
      
      <View className="flex-row items-center">
        <View className="mr-4">
          {isLoading ? (
            <View className={`rounded-full h-16 w-16 items-center justify-center border-2 ${getPartyColor(politician.party).split(' ')[0]}`}>
            ...
            <View className={`rounded-full h-16 w-16 items-center justify-center font-bold border ${getPartyColor(politician.party)}`}>
        ...
        <View className="flex-1">
          <View className={`rounded-md px-2 py-1 mb-1 self-start ${getPartyBadgeClass(politician.party)}`}>
            <Text className="text-white text-xs font-medium">{politician.party}</Text>
          </View>
