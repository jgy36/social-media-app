    ...
    const fetchCommunities = async () => {
      ...
      try {
        ...
        withTrending.forEach(community => {
          statusMap[community.id] = Boolean(community.isJoined);
        });
        ...
        if (process.env.NODE_ENV === 'development') {
          ...
          // Initialize joined status with mock data
          const statusMap: Record<string, boolean> = {};
          MOCK_COMMUNITIES.forEach(community => {
            statusMap[community.id] = false; // Start with all not joined in mock data
          });
          setJoinedStatus(statusMap);
              ...
              >
                <CardContent className="p-4">
                  <div className="flex justify-between items-start">
                    ...
                    <Button
                      variant={joinedStatus[community.id] ? "outline" : "default"}
                      size="sm"
                      className={joinedStatus[community.id] ? "border-primary/50" : ""}
